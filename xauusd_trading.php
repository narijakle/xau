...
<body class="bg-slate-950 text-slate-100 min-h-screen flex flex-col">
    ...
    <main class="max-w-7xl w-full mx-auto p-3 sm:p-4 space-y-4 flex-1">
        ...
        <div class="bg-slate-900/80 border border-slate-800 rounded-xl overflow-hidden shadow-xl flex flex-col">
            ...
            <!-- Candle Depth Toolbar (Scalper Mode) -->
            <div id="scalper-depth-bar" class="hidden flex flex-wrap items-center justify-between gap-2 px-3 sm:px-4 py-1.5 bg-slate-950 border-b border-slate-800/80 text-xs font-mono">
                <div class="flex flex-wrap items-center gap-2">
                    <span class="text-slate-400 text-[11px] font-semibold">📊 Jumlah Candle:</span>
                    <div class="flex items-center p-0.5 rounded-lg bg-slate-900 border border-slate-800">
                        <button type="button" onclick="setCandleLimit(120)" id="btn-php-lim-120" class="px-2.5 py-1 rounded text-xs font-semibold bg-purple-600 text-white shadow">120 Bar</button>
                        <button type="button" onclick="setCandleLimit(300)" id="btn-php-lim-300" class="px-2.5 py-1 rounded text-xs font-semibold text-slate-400 hover:text-white">300 Bar</button>
                        <button type="button" onclick="setCandleLimit(500)" id="btn-php-lim-500" class="px-2.5 py-1 rounded text-xs font-semibold text-slate-400 hover:text-white">500 Bar</button>
                        <button type="button" onclick="setCandleLimit(1000)" id="btn-php-lim-1000" class="px-2.5 py-1 rounded text-xs font-semibold text-slate-400 hover:text-white">1000 Bar</button>
                    </div>
                    <button type="button" onclick="loadMoreCandlesPHP()" id="btn-php-loadmore" class="flex items-center gap-1.5 px-3 py-1 rounded-lg text-xs font-bold text-amber-300 bg-amber-950/40 border border-amber-600/50 hover:bg-amber-900/60 hover:text-white transition-all">
                        ⏪ Muat +200 Bar (Geser Kiri)
                    </button>
                </div>
                <div class="flex items-center gap-2 text-[11px] text-slate-400">
                    <span class="hidden md:inline text-slate-500 text-[10px]">💡 Geser chart ke kiri untuk melihat riwayat lampau</span>
                    <button type="button" onclick="if(chart) chart.timeScale().scrollToRealTime();" class="px-2.5 py-1 rounded text-[11px] font-medium bg-slate-800 text-slate-300 hover:text-white hover:bg-slate-700 transition-colors">
                        🎯 Ke Candle Terkini
                    </button>
                </div>
            </div>
    ...
    <script>
        ...
        function setCandleLimit(limit) {
            currentLimit = limit;
            [120, 300, 500, 1000].forEach(num => {
                const btn = document.getElementById(`btn-php-lim-${num}`);
                if (btn) {
                    btn.className = (num === limit) 
                        ? "px-2.5 py-1 rounded text-xs font-semibold bg-purple-600 text-white shadow"
                        : "px-2.5 py-1 rounded text-xs font-semibold text-slate-400 hover:text-white";
                }
            });
            fetchRealKlines(currentSymbol, currentLimit);
        }

        async function loadMoreCandlesPHP() {
            if (isLoadingMorePHP) return;
            isLoadingMorePHP = true;
            currentLimit = Math.min(currentLimit + 200, 1500);
            const btn = document.getElementById('btn-php-loadmore');
            if (btn) btn.innerText = 'Memuat...';
            try {
                await fetchRealKlines(currentSymbol, currentLimit);
            } finally {
                isLoadingMorePHP = false;
                if (btn) btn.innerText = '⏪ Muat +200 Bar (Geser Kiri)';
            }
        }
        ...
        function switchChartView(mode) {
            ...
            const scalperDepthBar = document.getElementById('scalper-depth-bar');
            ...
            if (mode === 'SCALPER') {
                ...
                scalperDepthBar?.classList.remove('hidden');
                ...
                scalperDepthBar?.classList.add('hidden');
        ...
        function initChart() {
            ...
            // Auto-load more candles when dragging left
            chart.timeScale().subscribeVisibleLogicalRangeChange(range => {
                if (range && range.from <= 2 && !isLoadingMorePHP && currentLimit < 1500) {
                    loadMoreCandlesPHP();
                }
            });
        ...
        // Ambil data rill
        async function fetchRealKlines(symbol, limit = currentLimit) {
            const binanceSym = symbol === 'XAUUSD' ? 'PAXGUSDT' : 'BTCUSDT';
            const urls = [
                `/api/klines?symbol=${symbol}&provider=${currentBroker}&limit=${limit}`,
                `?action=klines&symbol=${symbol}&provider=${currentBroker}&limit=${limit}`,
                `https://data-api.binance.vision/api/v3/klines?symbol=${binanceSym}&interval=1m&limit=${limit}`,
                `https://api.binance.com/api/v3/klines?symbol=${binanceSym}&interval=1m&limit=${limit}`
            ];
        ...
        function evaluateScalper() {
            ...
            const lookback = 6;
            const rr = 1.3;
            const riskPct = 0.04; // 4% risk untuk modal $28
            ...
            for (let i = 25; i < rawCandles.length; i++) {
                ...
                if (activeTrade) {
                    if (activeTrade.type === 'BUY') {
                        // Trailing Breakeven (+0.2R) saat progres 55%
                        if (!activeTrade.isBreakevenSecured && candle.high >= activeTrade.entryPrice + (activeTrade.takeProfit - activeTrade.entryPrice) * 0.55) {
                            activeTrade.stopLoss = parseFloat((activeTrade.entryPrice + (activeTrade.takeProfit - activeTrade.entryPrice) * 0.2).toFixed(2));
                        ...
                        if (candle.high >= activeTrade.takeProfit) {
                            ...
                            const tradePnl = isSecured ? parseFloat((activeTrade.rewardAmount * 0.2).toFixed(2)) : -activeTrade.riskAmount;
                            activeTrade.status = isSecured ? 'TP_HIT' : 'SL_HIT';
                            activeTrade.exitPrice = activeTrade.stopLoss;
                            activeTrade.pnl = tradePnl;
                            currentBalance = Math.max(1, currentBalance + tradePnl);
                            activeTrade.balanceAfter = currentBalance;
                            tradesHistory.push({ ...activeTrade, exitTime: candle.time, exitReason: isSecured ? 'BE' : 'SL' });

                            // Marker STOP LOSS = ORANYE / BE = UNGU
                            markers.push({
                                time: candle.time,
                                position: 'belowBar',
                                color: isSecured ? '#a855f7' : '#f97316',
                                shape: 'circle',
                                text: isSecured ? `🛡️ BE PROTEKSI +$${tradePnl.toFixed(2)} (UNGU)` : `🟠 SL HIT -$${Math.abs(tradePnl).toFixed(2)} (ORANYE)`
                            });
                            activeTrade = null;
                        }
                    } else if (activeTrade.type === 'SELL') {
                        // Trailing Breakeven (+0.2R) saat progres 55%
                        if (!activeTrade.isBreakevenSecured && candle.low <= activeTrade.entryPrice - (activeTrade.entryPrice - activeTrade.takeProfit) * 0.55) {
                            activeTrade.stopLoss = parseFloat((activeTrade.entryPrice - (activeTrade.entryPrice - activeTrade.takeProfit) * 0.2).toFixed(2));
                            activeTrade.isBreakevenSecured = true;
                        }
                ...
                // Sinyal Masuk Baru: SMC Institutional Liquidity Sweep
                if (!activeTrade && i >= lookback && i < rawCandles.length - 1) {
                    const pastLows = rawCandles.slice(i - lookback, i).map(k => k.low);
                    const pastHighs = rawCandles.slice(i - lookback, i).map(k => k.high);
                    const minPastLow = Math.min(...pastLows);
                    const maxPastHigh = Math.max(...pastHighs);

                    const isBullSweep = candle.low < minPastLow && candle.close > minPastLow && candle.close > candle.open;
                    const isBearSweep = candle.high > maxPastHigh && candle.close < maxPastHigh && candle.close < candle.open;

                    const riskAmt = parseFloat((currentBalance * riskPct).toFixed(2));
                    const rewardAmt = parseFloat((riskAmt * rr).toFixed(2));
                    const slDist = currentSymbol === 'XAUUSD' ? 1.0 : 40.0;

                    if (isBullSweep) {
                        const entry = candle.close;
                        const sl = parseFloat((entry - slDist).toFixed(2));
                        const tp = parseFloat((entry + slDist * rr).toFixed(2));
                        activeTrade = {
                            id: `TR-${i}`,
                            type: 'BUY',
